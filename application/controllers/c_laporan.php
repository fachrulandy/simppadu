...
class C_laporan extends CI_Controller{
	...
	public function laporan_siup(){
		...
		if($this->input->post('cetak')){
			...
			foreach ($data as $r_data) {
				...
				$string_judul = "Daftar Surat Izin Usaha Perdagangan (SIUP) Bulan $bulan Tahun $tahun";
				if(!empty($id_kec_perusahaan)){
	...
	public function laporan_situ(){
		...
		if($this->input->post('cetak')){
			...
			if($tahun != '' or $bulan != ''){
				...
				if($tahun != ''){
					$sql .= "YEAR(t_situ.tanggal_terbit) = $tahun ";
				}
				...
				if($bulan != ''){
					$sql .= "MONTH(t_situ.tanggal_terbit) = $bulan ";
				}				
			}
	...
	public function laporan_sipl(){
		...
		if($this->input->post('cetak')){
			...
			if($tahun != '' or $bulan != ''){
				...
				if($tahun != ''){
					$sql .= "YEAR(t_sipl.tanggal_terbit) = $tahun ";
				}
				...
				if($bulan != ''){
					$sql .= "MONTH(t_sipl.tanggal_terbit) = $bulan ";
				}				
			}
			...
			foreach ($data as $r_data) {
				...
				$string_judul = "Daftar Surat Izin Pemindahan Limbah (SIPL) Bulan $bulan Tahun $tahun";
				if(!empty($id_kec_perusahaan)){
					$string_judul .= " Per Kecamatan ". ucwords(strtolower($this->m_kec->get_nm_kec($id_kec_perusahaan)));
				}
				$objPHPExcel->getActiveSheet()->setCellValue('C5', "   ".$string_judul);
			...
			$objPHPExcel->getActiveSheet()->setCellValue('I'.$h, "Bireuen, $tanggal_cetak");
			$h = $h + 1;
	...
	public function laporan_sipp(){
		...
		$this->load->view('templates/top');
        $this->load->view('laporan/sipp', $data);
        $this->load->view('templates/bottom');
	}
	...
	public function laporan_sikb(){
		...
		$this->load->view('templates/top');
        $this->load->view('laporan/sikb', $data);
        $this->load->view('templates/bottom');
	}
