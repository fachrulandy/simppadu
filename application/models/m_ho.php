...
class M_ho extends CI_Model{
    ...
    public function cetak_ho_baru($no_daftar){
        ...
        if($data->jenis_usaha == 'umum'){
            $umum_panjang = $data->umum_panjang;
            $umum_lebar   = $data->umum_lebar;
            $umum_lantai  = $data->umum_lantai;

            $ket_luas_tempat_usaha = $umum_panjang."M X ". $umum_lebar."M ";

            if($umum_lantai > 1){
                $ket_luas_tempat_usaha .= " X $umum_lantai ";
            }
            
            $total_luas = $data->total_luas;

            $ket_luas_tempat_usaha .= " = ". $total_luas ."M2";

            // add <w:br/> for newline

            
        }elseif($data->jenis_usaha == 'tower'){
            /*420 M2
            -   Lahan   : 20 M x 15 M = 300 M2
            -   Akses Jalan     : 40 M x 3 M = 120M2*/

            $tower_lahan_panjang       = $data->tower_lahan_panjang;
            $tower_lahan_lebar         = $data->tower_lahan_lebar;
            
            $tower_akses_jalan_panjang = $data->tower_akses_jalan_panjang;
            $tower_akses_jalan_lebar   = $data->tower_akses_jalan_lebar;
            
            $tower_tinggi              = $data->tower_tinggi;
            
            $total_luas                = $data->total_luas;

            $ket_luas_tempat_usaha  = $total_luas." M2 <w:br/>";
            $ket_luas_tempat_usaha .= "- Lahan : ". $tower_lahan_panjang ." M x ". $tower_lahan_lebar ." M = ". ($tower_lahan_panjang * $tower_lahan_lebar) ." M2 <w:br/>";
            $ket_luas_tempat_usaha .= "- Akses Jalan : ". $tower_akses_jalan_panjang ." M x ". $tower_akses_jalan_lebar ." M = ". ($tower_akses_jalan_panjang * $tower_akses_jalan_lebar) ." M2";

        }

        $template->setValue('luas_tempat_usaha', $ket_luas_tempat_usaha);
        
        if($data->jenis_usaha == 'tower'){
            $template->setValue('no_custom_1', '6.');
            $template->setValue('label_tinggi_tower', 'Tinggi Tower');
            $template->setValue('titik_dua_tinggi_tower', ':');
            $template->setValue('tinggi_tower', $data->tower_tinggi. 'M');
        ...
        $template->setValue('nilai_retribusi', 'Rp. ' . number_format( $data->nilai_retribusi, 0 , '' , '.' ) . ',-'); 
        ...
        $template->setValue('kec_pemilik', ucwords(strtolower($this->m_kec->get_nm_kec($data->id_kec_pemilik))));
        $template->setValue('kel_pemilik', ucwords(strtolower($this->m_kel->get_nm_kel($data->id_kel_pemilik))));
        ...
        $array_memperhatikan = explode('|', $data->memperhatikan);
        $num_rows            = count($array_memperhatikan);
        $template2->cloneRow('memperhatikan', $num_rows);
        
        
        $no = 1;

        for($i = 0; $i < $num_rows; $i++){
            if($array_memperhatikan[$i] != '' or !empty($array_memperhatikan[$i])){
                $template2->setValue("no#$no", $no); 
                $template2->setValue("memperhatikan#$no", $array_memperhatikan[$i]); 
            }
            $no++;
        }
        ...
        $template2->setValue('luas_tempat_usaha', $ket_luas_tempat_usaha);
        ...
        if($data->jenis_usaha == 'tower'){
            ...
            $template2->setValue('tinggi_tower', $data->tower_tinggi. 'M');
...
*/
        ...
        $tinggi_tower = $data->ho_baru_tower_tinggi;
        if($tinggi_tower > 0){
            $objPHPExcel->getActiveSheet()->setCellValue('L29', "Tinggi Tower : $tinggi_tower  M");
        }
        ...
        $nilai_retribusi_terbilang = strtoupper(terbilang($nilai_retribusi));
        $objPHPExcel->getActiveSheet()->setCellValue('D40', "$nilai_retribusi_terbilang Rupiah");
